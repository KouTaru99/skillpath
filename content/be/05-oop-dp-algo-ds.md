...
**Khác Ex·V3:** áp pattern ở **tầm kiến trúc** (nhiều module/nhà cung cấp), không chỉ trong phạm vi một class.

**Ví dụ thực tế — Adapter pattern gộp 3 cổng thanh toán khác nhau sau một hợp đồng chung.**
```java
interface PaymentGateway { PaymentResult charge(long amount, String orderId); }

class MomoAdapter implements PaymentGateway {
  public PaymentResult charge(long amount, String orderId) {
    MomoResponse res = momoSdk.createPayment(amount, orderId, momoSpecificFields);
    return new PaymentResult(res.getResultCode() == 0, res.getTransId());   // dịch response riêng của Momo
  }
}
// ZaloPayAdapter, VnPayAdapter tương tự — mỗi cái "dịch" SDK riêng về cùng 1 hợp đồng
```
Thêm cổng thanh toán thứ 4 chỉ cần viết thêm một Adapter, không sửa code checkout đang chạy.

**Vì sao là mức ④:** bạn dùng design pattern giải quyết bài toán kiến trúc thật (nhiều nhà cung cấp, dễ mở rộng) — không chỉ áp dụng pattern trong phạm vi một class.
